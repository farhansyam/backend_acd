@extends('layouts.app')
@section('title', 'Edit BP Service')
@section('page-title', 'Edit BP Service')
...
<div class="card border-0 mt-6">
    <div class="card-header border-b border-neutral-200 dark:border-neutral-600 bg-white dark:bg-neutral-700 py-4 px-6">
        <h6 class="text-lg font-semibold mb-0">Edit BP Service</h6>
    </div>
    <div class="card-body p-6">
        ...
        <form action="{{ route('bp-services.update', $bpService) }}" method="POST" enctype="multipart/form-data">
            @csrf @method('PUT')
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div>
                    ...
                    <select name="bp_id" class="form-control">
                        ...
                        @foreach($businessPartners as $bp)
                            <option value="{{ $bp->id }}"
                                {{ old('bp_id', $bpService->bp_id) == $bp->id ? 'selected' : '' }}>
                                {{ $bp->name }}
                            </option>
                        @endforeach
                    </select>
                    ...
                    <select name="service_type_id" class="form-control">
                        ...
                        @foreach($serviceTypes as $type)
                            <option value="{{ $type->id }}"
                                {{ old('service_type_id', $bpService->service_type_id) == $type->id ? 'selected' : '' }}>
                                {{ $type->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <div class="input-group">
                        ...
                        <input type="number" name="base_service"
                               value="{{ old('base_service', $bpService->base_service) }}"
                               class="form-control">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="input-group">
                        <input type="number" name="discount"
                               value="{{ old('discount', $bpService->discount) }}"
                ...
                <div class="md:col-span-2">
                    ...
                    @if($bpService->banner)
                        <div class="mb-3">
                            <p class="text-xs text-secondary-light mb-1">Banner saat ini:</p>
                            <img src="{{ asset('storage/' . $bpService->banner) }}"
                                 alt="banner" class="h-40 rounded-lg object-cover">
                        </div>
                    @endif

                    <input type="file" name="banner" id="banner" accept="image/*"
                           class="form-control" onchange="previewBanner(event)">
                    <p class="text-xs text-secondary-light mt-1">Kosongkan jika tidak ingin mengubah banner. Format: JPG, JPEG, PNG, WEBP. Maks 2MB.</p>

                    <div id="banner-preview" class="mt-3 hidden">
                        <p class="text-xs text-secondary-light mb-1">Preview baru:</p>
                ...
                <div class="flex items-center gap-3">
                    ...
                    <input type="checkbox" name="is_active" value="1"
                           class="w-5 h-5" {{ old('is_active', $bpService->is_active) ? 'checked' : '' }}>
                </div>
